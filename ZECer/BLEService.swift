...
class BLEService: NSObject, ObservableObject {
    ...
    // MARK: - Dynamic MTU + Size Header State
    private var dynamicMTU: Int = 182
    private var sizeHeaderParsed: Bool = false
    ...
    func startSending(data: Data) {
        // Prepend 4-byte big-endian UInt32 size header (size of payload, not including header)
        let size = UInt32(data.count)
        var prefixed = Data(count: 4)
        prefixed[0] = UInt8((size >> 24) & 0xFF)
        prefixed[1] = UInt8((size >> 16) & 0xFF)
        prefixed[2] = UInt8((size >> 8) & 0xFF)
        prefixed[3] = UInt8(size & 0xFF)
        prefixed.append(data)

        outgoingData = prefixed
        outgoingDataIndex = 0
        status = "Advertising..."
...
extension BLEService: CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if stringFromData == "EOM" {
            ...
            // Parse 4-byte big-endian size header from the first chunk
            if !sizeHeaderParsed && data.count >= 4 {
                let size = (UInt32(data[0]) << 24) | (UInt32(data[1]) << 16)
                         | (UInt32(data[2]) << 8)  |  UInt32(data[3])
                expectedTotalBytes = Int(size)
                sizeHeaderParsed = true
                incomingData.append(data.dropFirst(4))
            } else {
                incomingData.append(data)
            }

            // Real progress if we have the size, capped fake progress otherwise
            if expectedTotalBytes > 0 {
                progress = min(Double(incomingData.count) / Double(expectedTotalBytes), 0.95)
            } else {
                progress += 0.05
    ...
    private func finalizeReception() {
        ...
        case 4...:
            // "sigHex|||amount|||memo|||txHex"  (full signed packet)
            txHex  = components[3]
            amount = Double(components[1]) ?? 0.05
            memo   = components[2]
        case 3:
            // "amount|||memo|||txHex"
            txHex  = components[2]
            amount = Double(components[0]) ?? 0.05
            memo   = components[1]
        case 2:
            // legacy: "sig|||txHex"
            txHex  = components[1]
        ...
        DispatchQueue.main.async {
            TxManager.shared.saveIncoming(rawHex: txHex, amount: amount, memo: memo)
            self.status = "Success!"
...
extension BLEService: CBPeripheralManagerDelegate {
    ...
    // 2. Someone Subscribed -> Capture real MTU, START SENDING
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        dynamicMTU = central.maximumUpdateValueLength
        print("Receiver subscribed. MTU: \(dynamicMTU). Sending data...")
        sendData()
    }
    ...
    private func sendData() {
        ...
        while true {
            ...
            // Calculate size using dynamic MTU negotiated with central
            var amountToSend = dataToSend.count - outgoingDataIndex
            if amountToSend > dynamicMTU {
                amountToSend = dynamicMTU
            }
